...
`0.12.0` 已经完成 NewsNow `hotlist_fetch`、B站 yt-dlp `video_detail`、贴吧 aiotieba `forum_threads/post_detail`、公众号公开文章 `article_detail`、微博已知账号 `account_posts` 和知乎已知问答 `qa_detail` 的 raw-first artifact 闭环。当前阶段继续稳定数据源层，不在 Gateway 内加入长报告、多智能体研究或结论生成：
...
3. 微博已知账号第一页和知乎已知问答详情已经完成；继续为抖音、快手补已知账号或已知公开 URL 的低风险能力；
