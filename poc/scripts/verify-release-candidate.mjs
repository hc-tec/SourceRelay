...
try {
  ...
  await verifyReleaseBundle(releaseOutput, releaseBundle);
...
async function verifyReleaseBundle(root, manifest) {
  ...
  for (const entry of manifest.files) {
    ...
    const bytes = await releaseFile(root, entry.path);
  ...
  const sbom = JSON.parse((await releaseFile(root, manifest.sbom.file)).toString('utf8'));
  ...
  const checksums = JSON.parse((await releaseFile(root, 'sha256sums.json')).toString('utf8'));
  if (checksums.schemaVersion !== 1 || checksums.algorithm !== 'sha256' ||
      !Array.isArray(checksums.files) || JSON.stringify(checksums.excludes) !== JSON.stringify(['sha256sums.json'])) {
...
async function releaseFile(root, pathname) {
  const target = safeReleasePath(root, pathname);
