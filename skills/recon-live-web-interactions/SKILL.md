...
真实平台侦察的可见浏览器不是一次性脚本耗材。把连续“打开几秒就关闭”的窗口闪烁视为生命周期故障：它会破坏真实用户行为形态，也会增加平台对账号和设备的风险判断。

- 一个真实平台 run 只拥有一个可见、持久 browser context 和其目标页；基线、视觉复核、DOM、Network 和最多一次语义动作必须在同一会话内完成。
- 在发出平台导航后，DOM context 被替换、页面等待超时、selector 不成立或结果未知时，结束该动作链并保留目标页供复核；不得在同一 run 中新开浏览器、重开同一 URL、刷新或换一个临时 Profile 再试。
- 临时 Profile 仅可用于一个预先定义的匿名基线或离线本地测试；它必须承载完整计划而非一个采样步骤。若已打开真实平台页，不得连续创建多个 headed 临时 Profile 来调 selector 或等待条件。
