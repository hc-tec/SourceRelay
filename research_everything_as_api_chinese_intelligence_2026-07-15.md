...
### 2026-07-16：0.5.0 搜索到详情闭环

公开详情读取已注册为 `web.detail_fetch.trafilatura.v1`，并增加 `/tasks/search-and-fetch` 组合任务。Bing 搜索“国务院 政府工作报告”后，前三个结果均成功抽取正文，长度为 18,400、19,938 和 3,841 字；CSDN 搜索“FastAPI Agent”后，一篇抽取 3,493 字，另一篇因源站 HTTP 521 失败，整体保留为 `success + partial=true`。

这使底层能力从“需要时能搜”推进到“需要时能搜并读取公开详情”。第一版正文只处理公开 HTML/text，逐跳拒绝私网地址，不执行脚本、不使用登录 Profile，详情也不默认落库。需要浏览器渲染的详情页仍是下一类 Draft，而不是让公开下载器承担登录或风控绕过。

### 2026-07-16：0.6.0 浏览器渲染详情闭环

`detail_fetch` Draft 已实现并完成真实验证。它从公开样本 URL 提出标题与正文容器，只有最终主机仍在 `allowed_host`、标题非空、正文至少 200 字且不存在“正文不可用 + 认证门禁”时才允许晋升。运行时只读取容器 `innerText`，不点击、不提交表单、不读取 Cookie，也不自动展开或登录。

更重要的是，默认任务现在保持 direct-first：Trafilatura 总是第一层；BrowserWing 只有在平台注册了 verified/degraded、URL 又命中 recipe 主机时才成为第二层。真实知乎专栏页面中，Tier 1 收到 HTTP 403，Tier 2 成功读取样本文章 3,358 字和另一篇文章 1,916 字；正文容器还经过聚合内容惩罚，避免把推荐文章混入结果。搜索结果中的知乎首页因为主机不匹配而没有触发浏览器。三条组合结果为 `2 success + 1 no_results`，数据库前后不变。

这回答了 CLI-Anything/“万物皆接口”对当前需求的实际价值：关键不是把浏览器命令包装成 CLI，而是让每个自动化能力拥有动作类型、样本验证、主机范围、质量门槛、可靠性状态、fallback 顺序和逐条执行证据。浏览器只是一种 executor；真正构成底层能力的是经过验证的 Capability 生命周期。
