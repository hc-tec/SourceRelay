...
Unit (Vitest, 5 projects)
  ...
  gateway-domain
    -> Account Safety、stage terminal safety、Evidence 去敏/幂等/重启恢复、Profile binding、Task preflight ownership
...
Integration (@playwright/test project: integration)
  ...
  -> two production MV3 Profiles isolate Browser sessions, Native Messaging registrations, page leases, controller disconnect and close lifecycle
...
E2E (@playwright/test project: e2e-local)
  ...
  -> Gateway restart keeps each managed Profile's Host/browser/session alive
  -> cross-site profile mutation is rejected; Profile close is isolated and explicit
```
...
`verify:collector` 中，Playwright 已经执行过的 `browser-host-lifecycle`、`browser-host-extension-runtime-rejection`、`browser-host-multi-profile-isolation`、`strategy-binding`、`gateway-host-integration` 会从旧 `verify:local` 脊柱跳过，因此 aggregate 不会为了同一语义再启动一轮 Chromium。单独执行 `npm run verify:local` 时，这些 focused lane 仍可独立运行。
...
验证边界保持不变：上述 53 个 Unit 测试和 6 个 Playwright 本地 spec 都没有访问平台页面；它们证明的是正式本地执行面和安全不变量。新增的 runtime mismatch gate 用真实 production MV3，但向 Host 故意声明错误版本；它必须返回 `collector_extension_worker_version_mismatch`，并证明失败发生在正式 Profile 注册、Native Messaging 安装和 `profile_launched` journal 写入之前，随后清理 test-scoped Chromium/Host 进程。多 Profile gate 则实际启动两个独立的 production MV3/Chromium 会话，证明 Browser session、Native Messaging 注册和 Profile-local lease 不会串线；控制连接断开时两个 active lease 都必须被 quarantine，而两个浏览器与 bridge 保持，随后关闭其中一个也不会关闭另一个。Gateway E2E 将同一隔离约束再穿过实际 HTTP API 与 Gateway restart：两条 Profile 都必须保留各自 PID/session/bridge，关闭一条不得影响另一条。Unit 还补齐了 Evidence Plan 的预算、同意、Profile、权限与策略成熟度决策，以及 Native Bridge 的 worker 身份、命令结果关联、断线与 stale-session cleanup；后者还修复了延迟清理可能误断新 worker 的真实竞态。Gateway Task 的 rate-limit receipt 与 stage deadline 也必须收束 Account Safety，分别锁定或结束 active run，且不再继续派发。coverage 上升只是这些核心行为被明确断言后的副产物，不是单独目标。B站、小红书、知乎等页面的 DOM、可信鼠标输入、菜单、字幕、评论与 XHR 语义，仍只能由低频、可见、去敏、受预算的 L4 Canary 证明。
