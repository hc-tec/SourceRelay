...
| `danmaku` | 2026-07-23 独立真实侦察证明播放器覆盖层和“弹幕列表”均可读；列表行含 `data-index`、时间、文本、发送时间，虚拟长列表约 199 个位置；`/x/v2/dm/web/view` 与 `/x/v2/dm/wbi/web/seg.so` 为 `application/octet-stream` 二进制响应，未读取 body | 仍为 research-only；DOM-first 有界列表采集、内嵌可信滚动、artifact、真实 managed-profile canary、完整性和 admission 尚未完成；不能把覆盖层采样当成全量 |
