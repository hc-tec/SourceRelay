...
| capability | 当前可证明状态 |
|---|---|
| `native_search` | 历史 v1 曾完成匿名首个渲染页、最多 20 条的 admission；当前编译路径为 `bilibili.search.breadth.dom.v2`，已收敛为综合相关性或视频相关性/最新发布、页码 1–2、每次一页最多 20 条的本地契约与 production MV3 DOM 投影。2026-07-23 以 UTF-8 中文关键词“人工智能”真实完成视频/最新发布/第 1 页、20 条可见 BVID 卡片闭环，登录浮层存在但不影响公开结果。v1 的实证不会自动覆盖 v2，故当前 v2 仍为 `build_ready`，待第二页、综合/空结果样本与独立 review 后再决定 admission。详见[原生搜索 DOM v2 真实闭环](../validation/bilibili-native-search-dom-v2.0.0.md)。 |
