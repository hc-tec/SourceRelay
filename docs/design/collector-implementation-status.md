...
2026-07-29 小红书核心 direct 能力登记更新：`xiaohongshu.search.public_notes.v1`、`xiaohongshu.note.public_detail.v1`、`xiaohongshu.note.public_comments.v1` 与 `xiaohongshu.note.public_comment_replies.v1` 已有真实 Gateway→Extension 闭环；`xiaohongshu.account.public_notes.v1` 现支持两种执行目标：已有公开主页 tab 的零导航有界采集，以及上层一次性提供短时公开主页链接的单次导航采集。临时链接路径只接受公开主页 URL，在同一已有小红书 tab 内导航一次，Network/XHR 投影优先、可见 DOM 补充，最多 20 次可信滚动和 200 条公开笔记，连续两次无新增即停止；完整 URL、签名、响应正文和响应 URL 不进入 artifact 或长期档案。已有主页 tab 模式仍保持最多 3 次滚动和 40 条投影。临时链接路径的代码、契约、OpenAPI 和离线门禁已通过，但由于需要用户提供真实的短时主页链接，独立 live canary 仍为 `implementation_ready_live_e2e_pending`，不能把离线门禁冒充平台能力证明。
