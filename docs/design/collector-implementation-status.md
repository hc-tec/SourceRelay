...
| B站 discovery | `v1.1.0 live_anonymous_verified`，正式闭环已验证；`v2 @ 0.2.0` 已真实验证 UTF-8 视频/最新发布第 1–2 页、综合/相关性第 1 页、空结果终止，并完成最多两页的 batch manifest、合并、页间 BVID 去重和 checkpoint/resume 本地契约 | v2 仍不可 admission；补跨任务漂移/重复比例、真实中途终止恢复和独立 review；detail 策略不得复用 breadth admission |
...
P2a  Collection / Validation Profile launcher（完成）
  ...
  -> B站原生搜索 batch 的漂移/真实中途终止恢复与独立 review
