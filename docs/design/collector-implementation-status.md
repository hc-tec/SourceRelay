...
同日另一条全新短链 canary 暴露了多主页候选下的真实边界：短链导航已经成功到达目标主页，但旧扩展在全局查询公开 profile tabs 时返回 `xiaohongshu_public_profile_tab_ambiguous`，没有生成 artifact、没有重试或新 tab。修复后改为让短链 work 内部绑定刚刚导航的 tab，再核对该 tab 的 document；已有主页 tab 模式仍坚持唯一公开主页前置条件。新构建指纹为 `5bfc2e447312699caa018d6310e23cf1cc608c03f755d3b75058b72fa2501a4b`，纯逻辑门禁已通过，但由于失败链接已经消费，修复后的 multi-profile 真实 canary 仍需另一条全新短时链接，不能把本次修复误报成已通过真实回归。
