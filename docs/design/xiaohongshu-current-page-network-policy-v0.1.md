...
小红书公开笔记中的图片和视频地址可能带有短期签名、时效参数或其他临时访问上下文。上层 artifact 不能把这些链接当成可长期复用的媒体资产；仅保存 URL 会导致后续 AI 分析、证据复核或离线查看时内容已经失效。

后续应新增独立的媒体物化能力，不与当前搜索、详情、评论或回复能力混在一起：

1. 只处理当前已授权公开笔记能力真实观察到、且页面上公开可见的媒体；
2. 在本次短时采集窗口内有界下载图片或视频正文，优先复用浏览器已经产生的合法 Network 响应，不导出 Cookie、Token、header 或临时 URL；
3. 将正文写入本地内容寻址存储，上层只保存对象 ID、SHA-256、MIME、字节数、图片尺寸或视频时长、采集时间、来源 capability 与状态；
4. 临时媒体 URL、query、response header 和认证上下文不得进入长期 artifact；
5. 下载失败时记录 `expired | unavailable | too_large | risk_stopped | network_failed` 等明确状态，不通过刷新、重复打开详情、新建 tab 或无界重试补救；
6. 为单笔记媒体数、单文件大小、总字节数、并发数和下载 deadline 设置独立预算；视频与图片可以采用不同上限；
