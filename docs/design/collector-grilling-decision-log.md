...
第 1–100 题核心产品问询已经完成，跨决策冲突、覆盖缺口和术语一致性审计也已完成。后续决定优先级为：明确用户自定义选择 > 较晚批次 > 较早批次；具体场景决定 > 总体默认。当前没有仍需用户选择的阻塞性产品问题。开始实现仍需用户明确授权，现有代码不得被误写成已经满足本账本。
...
- 断网、导航失败、弱网和连续 Fetch/XHR failure 终止本轮并进入冷却，不自动续跑；
...
- 认证实网研究在[账号安全熔断门禁](account-safety-circuit-breaker.md)完成且用户再次明确批准前保持暂停。
