...
- 用户自有浏览器 direct work 的 claim 携带当前扩展 build fingerprint（仅作审计）；扩展在用户端绝不自动 `chrome.runtime.reload()`，Gateway 也不因 fingerprint 不匹配拒绝旧 worker——扩展更新在用户手动重载或重启浏览器后生效。
- Windows 下通过 MCP/Node 做真实平台验收时，非 ASCII 查询词必须经 UTF-8 安全通道传递（优先环境变量或显式 UTF-8 编码的输入），禁止把中文直接从 PowerShell 原生管道喂给 Node；提交 Tool 前先用 UTF-8 字节校验确认查询没有变成 `?`，否则不得产生平台动作。
- 这条门禁必须接在实际运行的 `poc/collector-gateway/dist/user-browser-server.js`；更新时先把生产 `collector-extension/dist` 制品同步到受管扩展目录。用户端扩展不自动 reload，更新靠用户手动重载或重启浏览器；CDP 自动 Reload 仅限仓库管理的验证/研究 Profile，禁止附着日常浏览器。
